function [wall,allbounds,MTpos,plusdir,mindir,nMTdomain,nMTplus,nMTminus] = getConfigWallsModified(x_coords,marker_positions,snaptol,domainends)
...
% nMTplus: 1 x n cell array, number of plus-MTs in each region
% MTstartbound: for each MT, which is its leftmost boundary
% MTendbound: for each MT, which is its rightmost boundary

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% THIS BLOCK HERE IS FOR WHEN WE WANT ALL MT TO BE USED IN CALCULATIONS. FILTERED IS CALCULATED BUT ABANDONED.
% nMT = length(x_coords);
% 
...
%% THIS BLOCK HERE IS FOR WHEN WE WANT THE NONE LABELED MT TO BE REMOVED IN CALCULATIONS
%% Original
nMT_original = length(x_coords);
% Original MT positions, including 'none'
MTpos_original = vertcat(x_coords{:});


% Remove inactive / nonexistent MTs
valid_indices = ~strcmp(marker_positions, 'none');

% Logical indexing to filter out 'none' marker positions
x_coords_active = x_coords(valid_indices);
marker_positions_active = marker_positions(valid_indices);

nMT_active = length(x_coords_active);

% covert to more convenient datatypes
% nx2 matrix of start and and points for each MT
[MTpos, ind] = sortrows(vertcat(x_coords_active{:}));

% list of logical values. 1 when MT points in the positive direction
plusdir = cellfun(@(x) strcmp(x,'right'), marker_positions_active);
plusdir = plusdir(ind)';

mindir = cellfun(@(x) strcmp(x,'left'), marker_positions_active);
mindir = mindir(ind)';
...
% nearby values
if abs(snaptol) > 0
    allbounds_low = uniquetol(MTpos(:),snaptol/max(abs(MTpos(:))),'lowest');
    allbounds_high = uniquetol(MTpos(:),snaptol/max(abs(MTpos(:))),'highest');
    allbounds = (allbounds_low+allbounds_high)/2;
else
    allbounds = unique(MTpos(:));
end
% 
...
for bc = 1:nbound
    ...
    Lend = abs(MTpos(:,1)-allbounds(bc))<=snaptol + eps;
    % microtubules with right end on this boundary
    Rend = abs(MTpos(:,2)-allbounds(bc))<=snaptol + eps;
    % microtubules that cross over this boundary
    Cross = MTpos(:,1)<allbounds(bc)-(snaptol + eps) & MTpos(:,2)>allbounds(bc)+ (snaptol + eps);
...
%     wall(bc,2) = ~any(plusdir_filtered' & (Cross | Lend)); 
    wall(bc,2) = ~any(plusdir & (Cross | Lend));
     ...
     if bc < nbound
        % ORIGINAL MTs -- for total MT count nMTdomain includes all MT
        % including none
        Lend_original = ...
            abs(MTpos_original(:,1)-allbounds(bc)) <= snaptol + eps;
        Cross_original = ...
            MTpos_original(:,1) < allbounds(bc)-(snaptol + eps) & ...
            MTpos_original(:,2) > allbounds(bc)+(snaptol + eps);

        indomain_original = Cross_original | Lend_original;
        % Includes 'none'
        nMTdomain(bc) = nnz(indomain_original);

        % ACTIVE MTs -- for plus-MT count
        indomain_active = Cross | Lend;
        nMTplus(bc) = nnz(indomain_active & plusdir);
        nMTminus(bc) = nnz(indomain_active & mindir);
        nMTminus = nMTminus(:);
     end
    % if (bc < nbound)
    ...
    %         indomain = (Cross |Lend);
    %         nMTdomain(bc) = nnz(indomain);
    % 
    %         nMTplus(bc) = nnz(indomain & plusdir);
    %    % end
    % end
